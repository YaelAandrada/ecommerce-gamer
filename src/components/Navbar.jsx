...
const Navbar = () => {
    ...
    // CORRECCIÓN: Cambiar 'quality' por 'quantity'
        ...
        <nav className="bg-gray-800 text-white p-4 shadow-lg sticky top-0 z-50">
            <div className="container mx-auto flex justify-between items-center">
                <div className="flex items-center space-x-3">
                    ...
                    <Link to="/" className="text-xl font-bold from-blue-400 to-purple-500 bg-clip-text text-transparent">
                        NeonByte
                    </Link>
                </div>
                ...
                <div className="hidden md:flex space-x-8">
                    ...
                    {isAdmin && (
                        <Link to="/admin" className={`transition-colors duration-200 ${isActiveLink('/admin')}`}>
                            Admin
                        </Link>
                    )}
                    
                    {isLoggedIn && (
                        <Link to="/wishlist" className={`transition-colors duration-200 ${isActiveLink('/wishlist')}`}>
                            ❤️ Mis Favoritos
                        </Link>
                    )}
                </div>
                                    ...
                                    {cartItems.length === 0 ? (
                                        ...
                                        cartItems.map(item => (
                                            <div key={item.id} className="flex items-center space-x-3 py-3 border-b border-gray-700">
                                                ...
                                                {/* CORRECCIÓN: Cambiar 'removerFromCart' por 'removeFromCart' */}
                                                <button 
                ...
                <button 
                    className="md:hidden flex flex-col justify-center items-center w-8 h-8 transition-transform duration-300 hover:scale-110" 
                    onClick={toggleMenu}
                    aria-label="Menu principal"
                >
                    <span className={`bg-white h-1 w-8 rounded transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-1' : ''}`}></span>
                    <span className={`bg-white h-1 w-8 rounded my-1 transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`}></span>
                    <span className={`bg-white h-1 w-8 rounded transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-1' : ''}`}></span>
                </button>

                {/* Controles desktop (theme y usuario) */}
                    ...
                    {isLoggedIn ? (
                        <div className="flex items-center space-x-4">
                            <div className="flex items-center space-x-3 bg-gray-700 px-3 py-2 rounded-lg">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    {isAdmin && (
                                        <span className="text-xs text-yellow-400">Administrador</span>
                                    )}
                                </div>
                            </div>
                            <button
                        ...
                        <div className="flex space-x-3">
                            ...
                            <div className="relative group">
                                ...
                                <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-gray-700">
                                    <button 
            ...
            {/* Menú móvil - CORRECCIÓN PRINCIPAL */}
            <div className={`md:hidden absolute left-0 right-0 bg-gray-800 transition-all duration-300 shadow-2xl border-t border-gray-700 ${
                isMenuOpen ? 'top-full opacity-100 visible' : 'opacity-0 invisible'
            }`}>
                <div className="flex flex-col space-y-1 p-4 max-h-[80vh] overflow-y-auto">
                    <div className="flex items-center justify-between py-3 px-4 bg-gray-700 rounded-lg mb-2">
                        <div className="flex items-center space-x-3">
                            <span className="text-xl">🛒</span>
                            <span>Carrito</span>
                        </div>
                    ...
                    <Link 
                        to="/" 
                        className={`py-3 px-4 rounded-lg transition-colors ${isActiveLink('/')} ${location.pathname === '/' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
                    ...
                    {/* Categorías en móvil */}
                    <div className="py-3 px-4 rounded-lg hover:bg-gray-700 transition-colors">
                        ...
                        <div className="grid grid-cols-2 gap-2 ml-4">
                            {categories.map(category => (
                                <Link
                                    key={category.id}
                                    to={`/categoria/${category.slug}`}
                                    className="block py-2 px-3 hover:bg-gray-600 rounded-lg transition-colors text-sm"
                                    onClick={() => setIsMenuOpen(false)}
                                >
                                    {category.name}
                                </Link>
                    ...
                    <Link 
                        to="/about" 
                        className={`py-3 px-4 rounded-lg transition-colors ${isActiveLink('/about')} ${location.pathname === '/about' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
                        onClick={() => setIsMenuOpen(false)}
                    >
                        👥 Nosotros
                    </Link>
                    ...
                    {isAdmin && (
                        <Link 
                            to="/admin" 
                            className={`py-3 px-4 rounded-lg transition-colors ${isActiveLink('/admin')} ${location.pathname === '/admin' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
                            onClick={() => setIsMenuOpen(false)}
                        >
                            ⚙️ Admin
                        </Link>
                    ...
                    {isLoggedIn && (
                        <Link 
                            to="/wishlist" 
                            className={`py-3 px-4 rounded-lg transition-colors ${isActiveLink('/wishlist')} ${location.pathname === '/wishlist' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
                            onClick={() => setIsMenuOpen(false)}
                        >
                            ❤️ Mis Favoritos
                        </Link>
                    ...
                    {isLoggedIn ? (
                        <div className="flex flex-col space-y-2">
                            ...
                            <button
                                onClick={handleLogout}
                                className="bg-red-500 hover:bg-red-600 px-4 py-3 rounded-lg transition-colors duration-200 flex items-center space-x-2 justify-center"
                            >
                                <span>🚪</span>
